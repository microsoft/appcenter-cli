 ...
 */
function _getUserEmailSettings(options, callback) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'v0.1/user/notifications/emailSettings';
 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _getAppEmailSettings(ownerName, appName, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
  ...
  // Construct URL
  let baseUrl = this.client.baseUri;
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'v0.1/apps/{owner_name}/{app_name}/notifications/emailSettings';
...
class Notifications {
  ...
  constructor(client) {
    this.client = client;
    this._getUserEmailSettings = _getUserEmailSettings;
    this._getAppEmailSettings = _getAppEmailSettings;
  }
   ...
   */
  getUserEmailSettingsWithHttpOperationResponse(options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._getUserEmailSettings(options, (err, result, request, response) => {
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  getUserEmailSettings(options, optionalCallback) {
    let client = this.client;
    let self = this;
    ...
    if (!optionalCallback) {
      ...
      return self._getUserEmailSettings(options, optionalCallback);
    }
  }
   ...
   * @reject {Error} - The error object.
   */
  getAppEmailSettingsWithHttpOperationResponse(ownerName, appName, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._getAppEmailSettings(ownerName, appName, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  getAppEmailSettings(ownerName, appName, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._getAppEmailSettings(ownerName, appName, options, optionalCallback);
    }
  }
