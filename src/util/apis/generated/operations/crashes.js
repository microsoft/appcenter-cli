 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
function _searchCrashes(crashGroupId, ownerName, appName, search, options, callback) {
   /* jshint validthis: true */
  let client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
...
class Crashes {
   ...
   * @reject {Error} - The error object.
   */
  searchCrashesWithHttpOperationResponse(crashGroupId, ownerName, appName, search, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._searchCrashes(crashGroupId, ownerName, appName, search, options, (err, result, request, response) => {
        let httpOperationResponse = new msRest.HttpOperationResponse(request, response);
        httpOperationResponse.body = result;
        if (err) { reject(err); }
        else { resolve(httpOperationResponse); }
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  searchCrashes(crashGroupId, ownerName, appName, search, options, optionalCallback) {
    let client = this.client;
    let self = this;
    if (!optionalCallback && typeof options === 'function') {
      optionalCallback = options;
    ...
    if (!optionalCallback) {
      ...
      return self._searchCrashes(crashGroupId, ownerName, appName, search, options, optionalCallback);
    }
  }
