 ...
 */
function _create(deploymentName, ownerName, appName, options, callback) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'v0.1/apps/{owner_name}/{app_name}/deployments/{deployment_name}/uploads';
...
/** Class representing a CodePushDeploymentUpload. */
class CodePushDeploymentUpload {
  /**
   * Create a CodePushDeploymentUpload.
   ...
   */
  createWithHttpOperationResponse(deploymentName, ownerName, appName, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._create(deploymentName, ownerName, appName, options, (err, result, request, response) => {
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  create(deploymentName, ownerName, appName, options, optionalCallback) {
    let client = this.client;
    let self = this;
    ...
    if (!optionalCallback) {
      ...
      return self._create(deploymentName, ownerName, appName, options, optionalCallback);
    }
  }

}

module.exports = CodePushDeploymentUpload;
