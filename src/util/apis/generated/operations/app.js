 ...
 */
function _blockLogs(ownerName, appName, options, callback) {
  ...
  let requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'v0.1/apps/{owner_name}/{app_name}/devices/block_logs';
...
class App {
   ...
   */
  blockLogsWithHttpOperationResponse(ownerName, appName, options) {
    let client = this.client;
    let self = this;
    return new Promise((resolve, reject) => {
      self._blockLogs(ownerName, appName, options, (err, result, request, response) => {
   ...
   *                      {stream} [response] - The HTTP Response stream if an error did not occur.
   */
  blockLogs(ownerName, appName, options, optionalCallback) {
    let client = this.client;
    let self = this;
    ...
    if (!optionalCallback) {
      ...
      return self._blockLogs(ownerName, appName, options, optionalCallback);
    }
  }
