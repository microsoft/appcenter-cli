 ...
 *
 *                      {object} [result]   - The deserialized result object.
 *                      See {@link SampleDevices} for more information.
 *
 *                      {object} [request]  - The HTTP Request object if an error did not occur.
 *
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
Analytics.prototype.getSampleDevices = function (ownerName, appName, options, callback) {
  var client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
    options = null;
  ...
  // Construct URL
  var baseUrl = this.client.baseUri;
  var requestUrl = baseUrl + (baseUrl.endsWith('/') ? '' : '/') + 'v0.1/apps/{owner_name}/{app_name}/analytics/sample_devices';
  requestUrl = requestUrl.replace('{owner_name}', encodeURIComponent(ownerName));
  requestUrl = requestUrl.replace('{app_name}', encodeURIComponent(appName));
  ...
  return client.pipeline(httpRequest, function (err, response, responseBody) {
    ...
    if (statusCode === 200) {
      ...
      try {
        ...
        result = JSON.parse(responseBody);
        if (parsedResponse !== null && parsedResponse !== undefined) {
          var resultMapper = new client.models['SampleDevices']().mapper();
          result = client.deserialize(resultMapper, parsedResponse, 'result');
        }
      } catch (error) {
        var deserializationError = new Error(util.format('Error "%s" occurred in deserializing the responseBody - "%s"', error, responseBody));
...
Analytics.prototype.audienceNameExists = function (audienceName, ownerName, appName, options, callback) {
  ...
  try {
    ...
    if (audienceName !== null && audienceName !== undefined) {
      if (audienceName.match(/[a-zA-Z0-9\-_]+/) === null)
      {
        throw new Error('"audienceName" should satisfy the constraint - "Pattern": /[a-zA-Z0-9\-_]+/');
      }
    }
    if (ownerName === null || ownerName === undefined || typeof ownerName.valueOf() !== 'string') {
      throw new Error('ownerName cannot be null or undefined and it must be of type string.');
  ...
  return client.pipeline(httpRequest, function (err, response, responseBody) {
    ...
    var statusCode = response.statusCode;
    if (statusCode !== 204 && statusCode !== 402) {
      var error = new Error(responseBody);
      error.statusCode = response.statusCode;
      error.request = msRest.stripRequest(httpRequest);
      error.response = msRest.stripResponse(response);
 ...
 *                      {stream} [response] - The HTTP Response stream if an error did not occur.
 */
Analytics.prototype.createOrUpdateAudience = function (audienceName, ownerName, appName, definition, options, callback) {
  var client = this.client;
  if(!callback && typeof options === 'function') {
    callback = options;
    options = null;
  ...
  try {
    ...
    if (definition === null || definition === undefined || typeof definition.valueOf() !== 'string') {
      throw new Error('definition cannot be null or undefined and it must be of type string.');
    }
  } catch (error) {
    return callback(error);
  }
  var audience = new client.models['Audience']();
  try {
