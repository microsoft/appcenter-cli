...
export function buildErrorInfo(err: any, profile: Profile, command: AppCommand): { message: string; exitCode: number } {
  ...
  let helpMessage = `Further error details: For help, please send both the reported error above and the following environment information to us by going to https://appcenter.ms/apps and starting a new conversation (using the icon in the bottom right corner of the screen)${
    os.EOL
  }
