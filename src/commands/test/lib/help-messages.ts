...
export module Messages {
  export module TestCloud {
    export module Commands {
      ...
      export const PrepareCalabash =
        `Creates an artifacts directory with Calabash tests.${os.EOL}` +
      ...
      export const PrepareUITests =
        `Creates an artifacts directory with Xamarin UI Tests.${os.EOL}` +
        `Required external tools:${os.EOL}` +
      ...
      export const RunCalabash =
        `Starts a test run with Calabash tests.${os.EOL}` +
        `Required external tools:${os.EOL}` +
        `- Ruby ${os.EOL}` +
        `- Gem xamarin-test-cloud`;
      ...
      export const RunUITests =
        `Starts a test run with Xamarin UI Tests.${os.EOL}` +
        `Required external tools:${os.EOL}` +
        `- .NET Framework on Windows, Mono Runtime on OS X${os.EOL}` +
        `- NuGet package Xamarin.UITests, version 2.0.1 or higher`;
    ...
    export module Arguments {
      ...
      export const EspressoTestApkPath =
      ...
      export const UITestsKeyPassword =
      ...
      export const MergeNUnitXml =
        "Merge the xml files in to the <arg> file. Requires --test-output-dir to specify the directory where the <arg> file will be. This parameter won't work when you run using --async because we have to wait for test results";
