...
export default class ShowDistributionGroupCommand extends AppCommand {
  ...
  public async run(client: MobileCenterClient): Promise<CommandResult> {
    ...
    const distributionGroupMembersRequestResponse = clientRequest<models.DistributionGroupUserGetResponse[]>(
      (cb) => client.account.getDistributionGroupUsers(app.ownerName, app.appName, this.distributionGroup, cb));

    // creating releases information request
    const basicReleasesDetailsRequestResponse = clientRequest<models.BasicReleaseDetails[]>(
      (cb) => client.distribute.getReleasesForDistributionGroup(this.distributionGroup, app.ownerName, app.appName, cb));
    ...
    try {
      ...
      if (error === 404) {
        return failure(ErrorCodes.InvalidParameter, `distribution group ${this.distributionGroup} was not found`);
      } else {
        debug(`Failed to get list of distribution group members - ${inspect(error)}`);
        return failure(ErrorCodes.Exception, "failed to retrieve list of distribution group users");
      }
    }
    ...
    try {
      ...
      return failure(ErrorCodes.Exception, "failed to retrieve releases details for the distribution group");
    }
