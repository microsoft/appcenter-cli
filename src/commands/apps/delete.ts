...
export default class AppDeleteCommand extends AppCommand {
  ...
  async run(client: AppCenterClient): Promise<CommandResult> {
    ...
    if (confirmation) {
      const deleteAppResponse = await out.progress("Deleting app ...", clientRequest<models.AppResponse>(cb => client.account.apps.deleteMethod(app.appName, app.ownerName, cb)));
