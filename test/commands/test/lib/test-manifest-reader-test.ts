...
describe("TestManifestReader.readManifest", () => {
  const expectedManifest = new TestManifest(
    "1.0.0",
    "1.0.14",
    new TestRunFile("test/commands/test/sample-test-workspace/apps/app.txt", "app.txt", "Ignores", "app-file"),
    [
      new TestRunFile("test/commands/test/sample-test-workspace/lib/tests.rb", "index.rb", "Ignored", "test-file"),
      new TestRunFile(
      ...
      new TestRunFile("test/commands/test/sample-test-workspace/resources/ReadMe.txt", "resources/ReadMe.txt", "Ignored", "test-file"),
    ],
    new TestFrameworkData("uiTest", {
      testFixtures: [
        {
          name: "MyTestFixture",
          testMethods: ["VerifyFoo", "VerifyBar"],
        },
        {
          name: "AnotherTestFixture",
          testMethods: ["VerifyBuzz"],
        },
      ],
    })
  );
